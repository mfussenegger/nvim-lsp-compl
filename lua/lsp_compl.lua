...
---@param item lsp.CompletionItem
---@param fuzzy boolean
function M._convert_item(client_id, item, fuzzy, offset, nerd_font)
	...
	return {
		word = word,
		abbr = item.label,
		kind = nerd_font and kind_icons[kind] or kind,
		menu = item.detail or "",
		info = info,
		icase = 1,
		dup = 1,
...
---@param prefix string
function M.text_document_completion_list_to_complete_items(client_id, items, fuzzy, offset, prefix, nerd_font)
	if #items == 0 then
		return {}
	end
	local matches = {}
	for _, item in ipairs(items) do
		if not fuzzy and item.filterText and prefix ~= "" then
			if next(vim.fn.matchfuzzy({ item.filterText }, prefix)) then
				local candidate = M._convert_item(client_id, item, fuzzy, offset, nerd_font)
				table.insert(matches, candidate)
			end
		else
			table.insert(matches, M._convert_item(client_id, item, fuzzy, offset, nerd_font))
		end
	end
...
function M._convert_items(client_id, encoding, result, lnum, line, word_boundary, startbyte, fuzzy, nerd_font)
...
local function next_debounce(subsequent_debounce)
	...
	local ms_since_request = (vim.uv.hrtime() - completion_ctx.last_request) * ns_to_ms
...
local function insert_char_pre(handle)
	...
	if pumvisible then
		if completion_ctx.isIncomplete or handle.has_fuzzy then
			...
			if debounce_ms == 0 then
				...
				completion_timer = assert(vim.uv.new_timer(), "Must be able to create timer")
	...
	if not signature_timer and handle.signature_triggers[char] ~= nil then
		signature_timer = assert(vim.uv.new_timer(), "Must be able to create timer")
		...
		if command then
			...
			if fn then
				...
				if vim.tbl_contains(server_commands, command.command) then
					...
					vim.notify("Command not supported on client or server: " .. command.command, vim.log.levels.WARN)
				end
			end
		end
	end
...
function M.attach(client, bufnr, opts)
	...
	add_client(
		handle.signature_triggers,
		vim.tbl_get(client.server_capabilities, "signatureHelpProvider", "triggerCharacters")
	)
	add_client(
		handle.completion_triggers,
